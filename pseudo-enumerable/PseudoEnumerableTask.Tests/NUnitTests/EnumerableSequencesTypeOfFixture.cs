...
namespace PseudoEnumerableTask.Tests.NUnitTests
{
    ...
    internal class EnumerableSequencesTypeOfFixture<T>
    {
        ...
        public EnumerableSequencesTypeOfFixture(ICollection source, IEnumerable<T> expected, T item)
        {
        ...
        public void TypeOf_Add_New_Element_To_Source_Sequence_Lazy_Evaluation()
        {
            ...
            var actual = this.source.TypeOf<T>();
        ...
        {
            Assert.Throws<ArgumentNullException>(() => ((IEnumerable<T>)null).TypeOf<T>(), $"Source can not be null.");
        }
    }
}
