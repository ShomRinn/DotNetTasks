...
namespace Linq
{
    ...
    public static class GroupingData
    {
        ...
        public static IEnumerable<(int remainder, IEnumerable<int> numbers)> Grouping()
        {
            ...
            return numbers.GroupBy(num => num % 5).Select(group => (group.Key, group.AsEnumerable()));
        }
        ...
        public static IEnumerable<(string category, IEnumerable<string> productsName)> GroupByCategory()
        {
            ...
            return products.GroupBy(p => p.Category).Where(g => g.Count() <= 7).Select(g => (g.Key, g.Select(p => p.ProductName)));
        }
        ...
        public static IEnumerable<IGrouping<string, string>> NestedGroupByCustom()
        {
            ...
            return anagrams.GroupBy(word => word.Trim(), word => word.ToUpper(CultureInfo.InvariantCulture), new AnagramEqualityComparer());
        }
    }
}
