...
namespace Linq
{
    ...
    public static class ProjectionOperations
    {
        ...
        public static IEnumerable<(string upper, string lower)> SelectByCase()
        {
            ...
            return words.Select(word => (word.ToUpper(CultureInfo.InvariantCulture), word.ToLower(CultureInfo.InvariantCulture)));
        }
        ...
        public static IEnumerable<(int number, bool inPlace)> SelectWithIndex()
        {
            ...
            return numbers.Select((num, index) => (num, num == index));
        }
        ...
        public static IEnumerable<(string customerId, int orderId, decimal total)> SelectFromChildSequence()
        {
            ...
            return customers.SelectMany(
                customer => customer.Orders.Where(order => order.Total < 500.00m),
        ...
        public static IEnumerable<(string customerId, int orderId, string orderDate)> SelectManyWithWhere()
        {
            ...
            return customers.SelectMany(customer => customer.Orders
                .Where(order => order.OrderDate >= dateTime)
                .Select(order => (customer.CustomerId, order.OrderId, order.OrderDate.ToString("dd-MMM-yy", CultureInfo.InvariantCulture))));
        }
        ...
        public static IEnumerable<(string customerId, int orderId, decimal totalValue)> SelectManyWhereAssignment()
        {
            ...
            return customers.SelectMany(customer => customer.Orders.Where(order => order.Total > 2000.00m), (customer, order) => (customer.CustomerId, order.OrderId, order.Total));
        }
        ...
        public static IEnumerable<string> IndexedSelectMany()
        {
            ...
            return customers.SelectMany((customer, index) => customer.Orders.Select(order => $"Customer #{index + 1} has an order with OrderID {order.OrderId}"));
        }
    }
}
