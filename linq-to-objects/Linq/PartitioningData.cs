...
namespace Linq
{
    ...
    public static class PartitioningData
    {
        ...
        /// </summary>
        /// <returns>The first 3 orders from customers in Washington with date of orders.</returns>
        public static IEnumerable<(string customerId, int orderId, DateTime orderDate)> CustomersTake()
        {
            List<Customer> customers = Customers.CustomerList;

            return customers.Where(customer => customer.Region == "WA").SelectMany(customer => customer.Orders.Select(order => (customer.CustomerId, order.OrderId, order.OrderDate))).Take(3);
        }
        ...
        public static IEnumerable<(string customerId, int orderId, DateTime orderDate)> CustomersSkip()
        {
            ...
            return customers.Where(c => c.Region == "WA").SelectMany(c => c.Orders, (customer, order) => (customer.CustomerId, order.OrderId, order.OrderDate)).Skip(2);
        }
