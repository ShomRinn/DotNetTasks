...
namespace Linq
{
    ...
    public static class AggregationOperations
    {
        ...
        public static IEnumerable<(string customerId, int orderCount)> CustomersOrdersCount()
        {
            ...
            return customers.Select(c => (c.CustomerId, c.Orders.Length));
        }
        ...
        public static IEnumerable<(string category, int productCount)> ProductsInCategoryCount()
        {
            ...
            return from product in products group product by product.Category into g select (g.Key, g.Count());
        }
        ...
        public static IEnumerable<(string category, int totalUnitsInStock)> TotalUnitsInStock()
        {
            ...
            return from product in products group product by product.Category into g select (g.Key, g.Sum(p => p.UnitsInStock));
        }
        ...
        public static IEnumerable<(string category, decimal cheapestPrice)> GetCheapestPrice()
        {
            ...
            return from product in products group product by product.Category into g select (g.Key, g.Min(p => p.UnitPrice));
        }
        ...
        public static IEnumerable<(string category, decimal mostExpensivePrice)> GetMostExpensivePrice()
        {
            ...
            return from product in products group product by product.Category into g select (g.Key, g.Max(p => p.UnitPrice));
        }
        ...
        public static IEnumerable<(string Category, decimal averagePrice)> AveragePrice()
        {
            ...
            return from product in products group product by product.Category into g select (g.Key, g.Average(p => p.UnitPrice));
        }
        ...
        public static double SeededAggregate()
        {
            ...
            return attemptedWithdrawals.Aggregate(startBalance, (acc, withdrawal) => acc - withdrawal >= 0 ? acc - withdrawal : acc);
        }
    }
}
