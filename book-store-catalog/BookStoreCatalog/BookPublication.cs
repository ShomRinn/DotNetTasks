...
namespace BookStoreCatalog
{
 ...
 : this(new BookAuthor(authorName), title, publisher, published, bookBinding, new BookNumber(isbnCode))
        ...
        {
            this.Author = new BookAuthor(authorName, isniCode ?? throw new ArgumentNullException(nameof(isniCode)));
        }
        ...
        public BookPublication(BookAuthor author, string title, string publisher, DateTime? published, BookBindingKind bookBinding, BookNumber isbn)
        {
            if (title is null)
            {
                throw new ArgumentNullException(nameof(title), "empty");
            }

            if (publisher is null)
            {
                throw new ArgumentNullException(nameof(publisher), "null");
            }

            this.Author = author is not null ? author : throw new ArgumentNullException(nameof(author), "null");
            this.Title = !string.IsNullOrWhiteSpace(title) ? title : throw new ArgumentException("empty", nameof(title));
            this.Publisher = !string.IsNullOrWhiteSpace(publisher) ? publisher : throw new ArgumentException("empty", nameof(publisher));
            this.Published = published ?? throw new ArgumentNullException(nameof(published));
            this.BookBinding = bookBinding;
            this.Isbn = isbn is not null ? isbn : throw new ArgumentNullException(nameof(isbn), "null");
        }
